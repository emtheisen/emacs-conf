;;; my-org.el --- Org Mode Configuration -*- mode: emacs-lisp; lexical-binding: t; -*-
...
(eval-when-compile (defvar thunderbird-program)
                   (defvar org-capture-templates)
                   (defvar org-agenda-skip-deadline-if-done)
                   (defvar org-agenda-custom-commands))

(declare-function org-message-thunderlink-open "my-org")
...
link into a thunderlink and then invokes thunderbird."
...
(defun browse-url-at-point-firefox (&optional ARG)
  "Browse a URL at the point, optional ARG arguments."
  (interactive)
...
(let* ((variable-tuple
  ...
  (custom-theme-set-faces
   ...
   `(org-level-4 ((t (,@headline ,@variable-tuple :height 100))))
   `(org-level-3 ((t (,@headline ,@variable-tuple :height 110))))
   `(org-level-2 ((t (,@headline ,@variable-tuple :height 120))))
   `(org-level-1 ((t (,@headline ,@variable-tuple :height 130))))
   `(org-document-title ((t (,@headline ,@variable-tuple :height 140 :underline nil))))))
